defmodule Mix.Tasks.FontAwesome.Generate do
  @moduledoc """
  Generate Font Awesome components.

  1. Clone Font Awesome project.

      $ git clone https://github.com/FortAwesome/Font-Awesome.git

  2. Run the task.

      $ mix fontawesome.generate
  """
  use Mix.Task

  @shortdoc "Convert source SVG files into HEEX components. Run `git clone https://github.com/FortAwesome/Font-Awesome.git` first."
  def run(_) do
    Enum.each(["brands", "regular", "solid"], &loop_directory/1)

    Mix.Task.run("format")
  end

  defp loop_directory(folder) do
    src_path = "./Font-Awesome/svgs/#{folder}/"
    namespace = "FontAwesome.#{String.capitalize(folder)}"
    ...
    defmodule Icons.#{namespace} do
      ...
      ### Examples
        ...
        <Icons.FontAwesome.#{namespace}.home class="w-6 h-6" />
        <Icons.FontAwesome.#{namespace}.render icon="home" class="w-6 h-6" />
    ...
    dest_path = "./lib/icons/font_awesome/#{folder}.ex"

    unless File.exists?(dest_path) do
      File.mkdir_p("./lib/icons/font_awesome")
    end
